...
API_TOKEN = "5939181352:AAGoG3q5-vYX37Ns0TNNDmvgLGZ8AETcJ4I"
logging.basicConfig(level=logging.INFO)
tele_bot = Bot(token=API_TOKEN)
storage = MemoryStorage()
dp = Dispatcher(tele_bot, storage=storage)

loop = asyncio.get_event_loop()


intents = discord.Intents.all()  # задаем стандартные права боту
intents.message_content = True  # даем боту разрешение для чтения содержимого сообщения
...
async def p(ctx, cl):
    ...
    """Выводит уроки с сайта https://369.pelikan.online пользователю."""
...
@commands.dm_only()
async def s(ctx, name, surname, cl, password):
...
loop.create_task(bot.start('MTAxNTI4NDY3ODY4MjQ4ODg3Mg.GDBSJD.OcSrrf29vlrlgMntIadqMs0ZPxFH29pK53FhDk'))
