...
* ID
  ...
  Функция записывает данные в словарь `telegram_id` в виде: ID пользователя в Discord - ключ, ID пользователя в Telegram - значение. Возможность получать уведомления в telegram о смене пароля в вашем классе.
...
* Пароль для учеников
  ...
  Регистрирует пользователя на сервере как ученика. Создает личный кабинет ученика в категории его класса и выдает ему роли, для доступа к чатам класса (Ученик, Ученик класс).
